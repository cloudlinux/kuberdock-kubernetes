...
func copySymlink(symlink, srcDir, dstDir, baseSrc string) error {
	...
	if !filepath.IsAbs(p) {
		relS, err := filepath.Rel(srcDir, filepath.Dir(symlink))
		if err != nil {
			return err
		}
		oldname := filepath.Join(dstDir, relS, filepath.Base(p))
		newname := filepath.Join(dstDir, relS, filepath.Base(symlink))
