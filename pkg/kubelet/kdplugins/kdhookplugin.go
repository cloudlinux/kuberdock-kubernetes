...
func (p *KDHookPlugin) prefillVolumes(containerId string, container *api.Container, pod *api.Pod) error {
	...
	if len(volumes) == 0 {
		glog.V(4).Infof(">>>>>>>>>>> Prefilling volumes for %s(%s): no empty volumes found", container.Name, containerId)
		return nil
	}

	if err := p.fillVolumes(volumes, container.VolumeMounts, dockerContainer.GraphDriver.Data["LowerDir"]); err != nil {
...
func (p *KDHookPlugin) fillVolumes(volumes []api.Volume, volumeMounts []api.VolumeMount, lowerDir string) error {
	...
	for _, pair := range mounts {
		...
		if !isDirEmpty(srcDir) {
			...
			if out, err := exec.Command("bash", "-c", fmt.Sprintf("cp -a %s/* %s", srcDir, dstDir)).CombinedOutput(); err != nil {
...
func createVolume(spec volumeSpec) error {
	if _, err := os.Stat(spec.Path); err != nil {
		glog.V(4).Infof(">>>>>>>>>>> Dir %q doesn't exist", spec.Path)
	} else {
		glog.V(4).Infof(">>>>>>>>>>> Dir %q exists!", spec.Path)
	}
