...
func (p *KDHookPlugin) OnContainerCreatedInPod(containerID string, container *v1.Container, pod *v1.Pod) {
...
func (p *KDHookPlugin) prefillVolumes(containerID string, container *v1.Container, pod *v1.Pod) error {
...
func (p *KDHookPlugin) fillVolumes(volumes []v1.Volume, volumeMounts []v1.VolumeMount, lowerDir string, volumesToPrefill []string) error {
...
func getVolumePath(volume v1.Volume) (string, error) {
	if volume.RBD != nil {
...
func getCephPath(volume v1.Volume) (string, error) {
	type rbdMap struct {
...
func getVolumeSpecs(pod *v1.Pod) []volumeSpec {
...
func (p *KDHookPlugin) getFixedIP(pod *v1.Pod) (string, error) {
	tr := &http.Transport{
...
// or empty string with error, if can't get one
func (p *KDHookPlugin) getPublicIP(pod *v1.Pod) (string, error) {
...
func (p *KDHookPlugin) OnPodRun(pod *v1.Pod) {
...
func (p *KDHookPlugin) OnPodKilled(pod *v1.Pod) {
	if pod != nil {
		glog.V(3).Infof(">>>>>>>>>>> Pod %q killed", pod.Name)
		pods, err := p.kubeClient.Core().Pods(pod.Namespace).List(v1.ListOptions{})
		if err != nil {
