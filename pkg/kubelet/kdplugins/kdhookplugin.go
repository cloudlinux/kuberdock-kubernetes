...
func (p *KDHookPlugin) prefillVolumes(container *api.Container, volumes api.Volumes, lowerDir string) error {
	var mounts []struct {
		volumePath      string
	...
	for _, vm := range container.VolumeMounts {
		for _, v := range volumes {
			if vm.Name == v.Name {
				mounts = append(mounts, volumePair{volumePath: getVolumePath(v), volumeMountPath: mv.MountPath})
			}
		}
	}
	if len(mounts) == 0 {
