...
func (p *KDHookPlugin) prefillVolumes(containerID string, container *api.Container, pod *api.Pod) error {
	...
	if cfi, ok := pod.Annotations["copyFromImage"]; ok {
		if err := json.Unmarshal([]byte(cfi), &volumesToPrefill); err != nil {
			glog.V(4).Infof("Unable to parse copyFromImage annotation (%s): %q", va, err)
			return err
		}
	}
